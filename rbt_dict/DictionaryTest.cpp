...
int main() {
    ...
    string S[] = { // words to insert
        "plaices", "ambusher", "crosby", "wattles", "pardoner", "pythons", "second", "forms",
        "impales", "pic", "verticals", "recriminator", "depressants", "cul", "potables", "norm",
        "reinsurer", "deschooler", "recoining", "dissocialised", "cohabit", "hemiolia", "cantling",
        "glamorized", "millesimal", "glagolitic"
    };
    string T[] = {
        // words to delete
        "second",
    ...
    Dictionary test;
    Dictionary test2;
    assert(test == test2); // test empty equality
    string strings[] = {"hello", "bye", "freddy", "bonnie", "chica", "foxy"};
    for (int i = 0; i < 6; i++) {
        test.setValue(strings[i], i + 1);
    }
    assert(!(test == test2));
